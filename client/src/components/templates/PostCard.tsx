...
const PostCard: React.FC<PostCardProps> = ({ post, template = "basic" }) => {
  ...
  return (
    ...
    >
      ...
      <div
        className={`${isHoverTemplate ? "absolute bottom-0 left-0 right-0 opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-white p-4" : "mt-auto"}`}
      >
        <div className="flex flex-col justify-between gap-4 text-sm md:gap-2">
          ...
          <div
            className={`flex flex-row items-center min-w-0 overflow-hidden text-ellipsis whitespace-nowrap`}
          >
